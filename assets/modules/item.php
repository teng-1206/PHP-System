...
class Item_Controller 
{
    ...
    public function create( $conn, Item $object ) {
        $sql = "INSERT INTO item( name, description, status, amount, purchase_date, broken_date, image_url, thumb_image_url, fk_user_id )
                VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ? )";
        $stmt = $conn->prepare( $sql );
        $result = $stmt->execute( [
            $object->get( 'name' ),
    ...
    public function update( $conn, Item $object)  {
        $sql = "UPDATE item
                SET name = ?, description = ?, status = ?, amount = ?, purchase_date = ?, broken_date = ?, image_url = ?, thumb_image_url = ?, update_at = CURRENT_TIMESTAMP
                WHERE id = ?";
        $stmt = $conn->prepare( $sql );
        $result = $stmt->execute( [
            $object->get( 'name' ),
    ...
    public function convert ( array $object )
    {
        ...
        $new_object->set( 'thumb_image_url', $object[ 'thumb_image_url' ] );
